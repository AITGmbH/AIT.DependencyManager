...
{
    [TestClass]
    public class DependencyFileShareControlTest
    {
        ...
        public void Init()
        {
            ...
            FileSharePreparation.PrepareFileShareTestEnvironment(Values.WorkspaceOwner, Values.Password, Values.TeamProjectCollection, _workspaceName, Values.TeamProjectName, _localWorkspaceFolder, Values.FileShareFolder);
        }

        [TestMethod]
        [TestCategory("IntegrationTest")]
        public void FileShare_Resolver_Success()
        {
            // Arrange:
            ...
            // Act:
            var result = target.GetDependencyGraph(service.Object, _localWorkspaceFolder + @"\FileShareTarget\component.targets");
            ...
            Assert.AreEqual("Example1.dll", dllFiles.First());
        }
        ...
        public void Clean()
        {
            ...
            //Delete the content of the existing shared folder, but not the folder itself!
            if (Directory.Exists(Values.FileShareFolder))
            {
                var subFolders = Directory.GetDirectories(Values.FileShareFolder).Select(path => Path.GetFullPath(path)).ToArray();
