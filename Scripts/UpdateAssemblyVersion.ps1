...
## (c) Microsoft Corporation. This source is subject to the Microsoft Permissive License.
...
#$Env:BUILD_SOURCESDIRECTORY = "C:\TFS\online_AIT\AIT\Tools\TFS ASAP\OnPremise\2015.0\Main-2015.0\"
#$Env:BUILD_BUILDNUMBER = "Build HelloWorld_0000.00.00.0"
...
Write-Warning "Version: $NewVersion"
...
$files = gci $Env:BUILD_SOURCESDIRECTORY -recurse | 
	...
	if($files)
	{
		Write-Warning "Will apply $NewVersion to $($files.count) files."
...
# Apply the version to the vsix manifest file
$files = gci $Env:BUILD_SOURCESDIRECTORY -recurse | 
    ?{ $_.PSIsContainer } | 
    foreach { gci -Path $_.FullName -Recurse -include 'source.extension.vsixmanifest' }
	...
	if($files)
	{
		...
		foreach ($file in $files) 
		{
			foreach ($file in $files)
			{
				...
				{
					$node.Identity.Version = $NewVersion.Value
